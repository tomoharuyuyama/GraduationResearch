<x-app :tables="$tables">
    ...
    <div class="flex items-center justify-between mb-5 pb-5">
        <form action="{{ route('edit_column', ['tableId' => $selectedTable->id, 'columnId' => $selectedColumn->id]) }}" method="post" class="mb-5 w-1/2">
            @csrf
            <div class="flex mb-5">
                <input type="text" name="column_name" class="w-full border-slate-300 border-1 h-10 mb-5 rounded-md pl-5" value="{{ $selectedColumn->column_name }}">
                <button
